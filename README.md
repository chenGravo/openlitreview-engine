...
- 模型：DeepSeek、Kimi、豆包通过统一适配层调用；DeepSeek V4 Pro 负责证据综合与主写作，Kimi K2.6 在写作前审查矛盾证据、遗漏视角和教育—医学边界，豆包 Seed 2.1 Pro 在写作后独立复核。Kimi 是默认正式链路中的必要步骤，调用失败时不会静默生成缺少 Kimi 审查的成品。
